	...
	<div class="container-fluid-full">
		<div class="row-fluid">
			...
			<div id="content" class="span10">
				...
				<!-- 任务列表 -->
				<a href="input-schedule-job.shtml" class="btn btn-success"
					role="button">添加任务</a>
				<table class="table table-bordered table-margin-top">
					...
					<tr>
						...
						<td>$!{item.jobGroup}</td>
						<td>$!{item.jobTrigger}</td> #if ($!{item.status}=="PAUSED")
						<td class="status-color">$!{item.status}</td> #else
						<td>$!{item.status}</td> #end
						<td>$!{item.cronExpression}</td>
						<td>#if ($!{item.isSync}) 异步 #else 同步 #end</td>
						<td>$!{item.url}</td>
						<td>$!{item.description}</td>
						<td>
							<!--<a href="input-schedule-job.shtml?scheduleJobId=$!{item.scheduleJobId}">修改</a>-->
							<a class="btn btn-danger" role="button"
							href="pause-schedule-job.shtml?scheduleJobId=$!{item.scheduleJobId}"><i
								class="fa fa-pause-circle-o" aria-hidden="true"></i>暂停</a> <a
							class="btn btn-danger" role="button"
							href="resume-schedule-job.shtml?scheduleJobId=$!{item.scheduleJobId}"><i
								class="fa fa-reply" aria-hidden="true"></i>恢复</a> <a
							class="btn btn-danger" role="button"
							href="run-once-schedule-job.shtml?scheduleJobId=$!{item.scheduleJobId}"><i
							...
							class="btn btn-danger" role="button"
							href="delete-schedule-job.shtml?scheduleJobId=$!{item.scheduleJobId}"><i
