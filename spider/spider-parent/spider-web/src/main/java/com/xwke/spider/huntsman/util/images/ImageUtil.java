...
/**
 * 图片处理工具类 单例模式 Copyright 2014-2015 the original ql Created by QianLong on
 * 2014/7/9 0009.
 */
public class ImageUtil {
	...
	/**
	 * 图片转换操作 注意： 1、width与height成对出现才会生效 2、如果指定了width和height，那么scale参数将会失效
	 * 
	 * @param image
	 *            待转换的图像
	 * @param height_str
	 ...
	 * @return
	 */
	public static ImageConvert imageConvert(BufferedImage image, String height_str, String width_str, boolean sign,
			boolean recomment, float signAlpha, String scale_str, float alpha) {
		...
		if (height_str != null && width_str != null) {// 转换图像到指定高宽
			...
			try {
				...
				log.error("图像输出错误", e);
				image = ImageUtil.getInstance().getNoImageByConvert(height_str, width_str, sign, recomment, signAlpha,
						scale_str, alpha);
	...
	public void getImageAlpha(BufferedImage image, float alpha) {
		if (alpha != 1) {
			...
			g2.drawImage(image.getScaledInstance(image.getWidth(), image.getHeight(), Image.SCALE_SMOOTH), 0, 0, null);
	 ...
	 * @throws IOException
	 */
	public void writeImage(String fileType, BufferedImage image, ServletOutputStream servletOutputStream)
			throws IOException {
		try {
	 ...
	 * @throws IOException
	 */
	public void writeImage(String fileType, String imagePath, ServletOutputStream servletOutputStream)
			throws IOException {
		try {
	 ...
	 * @throws IOException
	 */
	private void imageWrite(String fileType, BufferedImage image, ServletOutputStream servletOutputStream)
			throws IOException {
		switch (fileType) {
		case ".jpg": {
	 ...
	 * @return
	 */
	public BufferedImage turnImageToWH(int width, int height, BufferedImage image, boolean sign, boolean recomment,
			float signAlpha) {
	...
	public BufferedImage scale(int scale, BufferedImage scaleImage, boolean sign, boolean recomment, float signAlpha) {
		if (scale != 100) {
			...
			g2.drawImage(scaleImage.getScaledInstance(width, height, Image.SCALE_SMOOTH), 0, 0, null);

			try {
				bufferedImage = Thumbnails.of(bufferedImage).scale(1f).outputQuality(0.38f).asBufferedImage();
			} catch (IOException e) {
			...
			try {
				...
				if (!sign && !recomment) {
					bufferedImage = Thumbnails.of(scaleImage).scale(1f).outputQuality(0.40f).asBufferedImage();
				}
				if (sign) {
					bufferedImage = addSign(bufferedImage, signAlpha);
				}
	...
	public BufferedImage getNoImage() {
		...
		try {
			...
			image = new BufferedImage(100, 35, BufferedImage.TYPE_INT_RGB);
	 ...
	 * @return
	 */
	public BufferedImage getNoImageByConvert(String height_str, String width_str, boolean sign, boolean recomment,
			float signAlpha, String scale_str, float alpha) {
		...
		if (request != null) {
			cacheFileName = ImageCatchUtil.getImageCachePathByFtpPath(path, sign, recomment, height_str, width_str,
					scale_str, signAlpha, alpha);
		...
		try {
			...
			String cacheDir = (webPath + path.substring(1, path.lastIndexOf("/") + 1)).replace("/", File.separator);
	...
	public BufferedImage addSign(BufferedImage image, float signAlpha) {
		...
		try {
			...
			g2.drawImage(markImg, image.getWidth() - markImg.getWidth() - 9,
					image.getHeight() - markImg.getHeight() - 9, null);
		} catch (IOException e) {
	...
	public BufferedImage addRecommentSign(BufferedImage image) {
		...
		// 在背景图中画上原图像
		g2.drawImage(image.getScaledInstance(image.getWidth(), image.getHeight(), Image.SCALE_SMOOTH), offset, offset,
				null);
